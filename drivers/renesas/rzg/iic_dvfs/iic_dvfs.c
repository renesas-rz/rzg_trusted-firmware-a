...
#define RZG_DVFS_API(__name, ...)					\
int32_t __attribute__ ((section (".system_ram"))) 			\
rzg_iic_dvfs_ ##__name(__VA_ARGS__)
...
#define RZG_DVFS_API(__name, ...)					\
int32_t rzg_iic_dvfs_ ##__name(__VA_ARGS__)
#endif
...
IIC_DVFS_FUNC(start, DVFS_STATE_T * state)
{
	...
	lsi_product = mmio_read_32(RZG_PRR) & PRR_PRODUCT_MASK;
	if (lsi_product == PRR_PRODUCT_G2E)
		goto start;

	reg = mmio_read_32(RZG_MODEMR) & CHECK_MD13_MD14;
	switch (reg) {
...
RZG_DVFS_API(send, uint8_t slave, uint8_t reg_addr, uint8_t reg_data)
{
...
RZG_DVFS_API(receive, uint8_t slave, uint8_t reg, uint8_t *data)
{
	DVFS_STATE_T state = DVFS_START;
	int32_t result = DVFS_PROCESS;
	uint32_t err = 0;
