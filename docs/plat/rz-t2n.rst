...
The "RZ/T2N" high-end 64-bit Arm®-based microprocessors (MPUs)
enables the solutions required for the smart society of the future.
Through a dual core of Arm Cortex®-A55 with the addition two R52 cores, engineers can
...
::
	...
	On-chip 64-bit Arm Cortex-A55 processor
 	Dual/Single MPCore cores
 	Maximum operating frequency:
	– Core: 600/1200 MHz
	– DSU: 500/1000 MHz
 	64-bit Arm Cortex-A55 Dual/Single MPCore cores (revision r2p0)
 	Address space: 32 Gbytes
 	L1 cache:
	– I-cache: 32 Kbytes (with parity) each core
	– D-cache: 32 Kbytes (with ECC) each core
 	L2 cache : 128 Kbytes (with ECC)
 	L3 cache : 256 Kbytes (with ECC)
 	NEON/FPU supported
 	Cryptographic extension supported (Security product only)
 	Arm V8.2-A architecture

	On-chip 32-bit Arm Cortex-R52 processor
 	High-speed realtime control with operating frequency of 400/800MHz
 	On-chip Dual 32-bit Arm Cortex-R52 (revision r1p4)
 	Tightly coupled memory (TCM) with ECC
	– CPU0, CPU1: 512 KB(ATCM)/64 KB(BTCM)
...
Renesas RZ/T2N reference platforms:
-----------------------------------

+--------------+---------------------------------------------------------------------------------------------------------------------------------------+
| Board        |      Details                                                                                                                          |
+==============+===============+=======================================================================================================================+
| evk         | Equipped with Renesas RZ/T2N SoC                                                                                                      |
|              +---------------------------------------------------------------------------------------------------------------------------------------+
...
On RZ/T2N SoCs the BOOTROM starts the cpu at EL3; for this port BL2
...
The current TF-A port has been tested on the RZ/T2N Evk board.

* u-boot:
  The port has been tested using mainline uboot with RZ/T2N Evk board specific patches.

* linux:
  The port has been tested using mainline kernel with the RZ/T2N Evk board.
...
the evk RZ/T2N development kit used in this port.
...
.. code:: bash
       ...
       make PLAT=t2n all BOARD=evk

Build Options:
~~~~~~~~~~~~~~

This is used to specify to build for the dual or single core SoC.
...
.. code:: bash
	...
	make PLAT=t2n BOARD=evk PLATFORM_CORE_COUNT=2 BL33=${path_to_uboot_file}/u-boot.bin bl2 fip pkg <Build Options>
...
.. code-block:: text
	...
	13. Write Linux files to the SD card
		sudo cp ./<t2n device tree>.dtb /media/user/79273262-4ff6-424f-9e7e-a
		sudo cp ./<t2n kernel image>.bin /media/user/79273262-4ff6-424f-9e7e-a
		sudo tar -jxvf <t2n root file system>.tar.bz2 -C /media/user/c18b1089-2298-40fe-b5eb-c
...
.. code-block:: text
	...
	NOTE: For "Reported Core Count (n) is different from PLATFORM_CORE_COUNT", n should be 1, or 2,dependent on the CPU variant.
