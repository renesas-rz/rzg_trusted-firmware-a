...
The "RZ/G2UL" high-end 64-bit Arm®-based microprocessors (MPUs)
enables the solutions required for the smart society of the future.
Through a single core of Arm Cortex®-A55 and in addition a M33 core, engineers can
...
::
    ...
    ARM CA55 (ARMv8) 1.0 GHz single core, with NEON/VFP, L1$ I/D 32K, L2$ 0MB, L3$ 256K
...
Renesas RZ/G2UL reference platforms:
------------------------------------

+--------------+-----------------------------------------------------------------------------------------------------------------------------------------+
| Board        |      Details                                                                                                                            |
+==============+===============+=========================================================================================================================+
| g2ul_smarc   | Equipped with Renesas RZ/G2UL SoC                                                                                                       |
|              +-----------------------------------------------------------------------------------------------------------------------------------------+
|              | https://www.renesas.com/jp/en/products/microcontrollers-microprocessors/rz-mpus/rzg2ul-evaluation-board-kit-rzg2ul-evaluation-board-kit |
+--------------+-----------------------------------------------------------------------------------------------------------------------------------------+
...
Overview
--------
On RZ/G2UL SoCs the BOOTROM starts the cpu at EL3; for this port BL2
...
The current TF-A port has been tested on the g2ul_smarc RZ/G2UL board.
SoC_id  R9A07G043U11GBG revision ESx.y.

* u-boot:
  The port has been tested using mainline uboot with the G2UL Board added.

* linux:
  The port has been tested using mainline kernel with the G2UL Board added.
...
the SMARC RZ/G2UL development kit used in this port.
...
.. code:: bash
    ...
    make PLAT=g2ul all BOARD=g2ul_smarc
...
.. code-block:: text
	...
	5. Then send the device the FlashWriter[1] (e.g. Flash_Writer_SCIF_RZG2UL_SMARC_DDR4_1GB_1PCS.mot).
...
.. code-block:: text
	...
	13. If Linux is required on this SD card, then follow the steps below.
		sudo cp ./<g2ul device tree>.dtb /media/user/79273262-4ff6-424f-9e7e-a
		sudo cp ./<g2ul kernel image>.bin /media/user/79273262-4ff6-424f-9e7e-a
		sudo tar -jxvf <g2ul root file system>.tar.bz2 -C /media/user/c18b1089-2298-40fe-b5eb-c
