...
static uintptr_t rz_otp_tsu_get_temp(void *handle, u_register_t x1)
{
	...
	if (is_rz_otp_tsu_offset((uint32_t) x1)) {
		mmio_write_32(RZT2_OTPPWR, (RZT2_OTPPWR_ACCL | RZT2_OTPPWR_PWR));
		while ((mmio_read_32(RZT2_OTPSTR) & 0x1) != 0x1)
			;

		mmio_write_32(RZT2_OTPADRRD, (uintptr_t) x1);
		val = mmio_read_32(RZT2_OTPDATARD);
		mmio_write_32(RZT2_OTPPWR, 0x0);
		while ((mmio_read_32(RZT2_OTPSTR) & 0x1) == 0x1)
			;
