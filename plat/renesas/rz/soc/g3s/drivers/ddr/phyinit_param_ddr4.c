/*
 * Copyright (c) 2023, Renesas Electronics Corporation. All rights reserved.
 *
 * SPDX-License-Identifier: BSD-3-Clause
 */

#include <stdint.h>
#include <stddef.h>
#include <lib/utils_def.h>

// Step (A) : Bring up VDD, VDDQ, and VAA 
// Step (B) Start Clocks and Reset the PHY 
// Step (C) Initialize PHY Configuration 
const	uint32_t	ddrphy_ini_data_c[][2] =
{
	{0x06E000,	0x00000000},
	{0x03005F,	0x000002f7},
	{0x03015F,	0x000002f7},
	{0x03105F,	0x000002f7},
	{0x03115F,	0x000002f7},
	{0x000055,	0x000000f2},
	{0x001055,	0x000000f2},
	{0x002055,	0x000000f2},
	{0x003055,	0x000000f2},
	{0x004055,	0x000003f2},
	{0x005055,	0x000000f2},
	{0x006055,	0x000000f2},
	{0x007055,	0x000000f2},
	{0x008055,	0x000000f2},
	{0x009055,	0x000000f2},
	{0x0580C5,	0x0000000b},
	{0x05802E,	0x00000001},
	{0x058024,	0x00000009},
	{0x05803A,	0x00000002},
	{0x05807D,	0x00000212},
	{0x05807C,	0x00000061},
	{0x058056,	0x0000000a},
	{0x03004D,	0x00000018},
	{0x03014D,	0x00000018},
	{0x03104D,	0x00000018},
	{0x03114D,	0x00000018},
	{0x030049,	0x00000e38},
	{0x030149,	0x00000e38},
	{0x031049,	0x00000e38},
	{0x031149,	0x00000e38},
	{0x000043,	0x00000021},
	{0x001043,	0x00000021},
	{0x002043,	0x00000021},
	{0x003043,	0x00000021},
	{0x004043,	0x00000021},
	{0x005043,	0x00000021},
	{0x006043,	0x00000021},
	{0x007043,	0x00000021},
	{0x008043,	0x00000021},
	{0x009043,	0x00000021},
	{0x058018,	0x00000001},
	{0x058075,	0x00000002},
	{0x058050,	0x00000011},
	{0x058008,	0x00000190},
	{0x058088,	0x00000009},
	{0x0580B2,	0x0000020c},
	{0x030043,	0x000005b1},
	{0x030143,	0x000005b1},
	{0x031043,	0x000005b1},
	{0x031143,	0x000005b1},
	{0x0580FA,	0x00000001},
	{0x058019,	0x00000004},
	{0x0580F0,	0x00005555},
	{0x0580F1,	0x00005555},
	{0x0580F2,	0x00005555},
	{0x0580F3,	0x00005555},
	{0x0580F4,	0x00005555},
	{0x0580F5,	0x00005555},
	{0x0580F6,	0x00005555},
	{0x0580F7,	0x0000f000},
	{0x03004A,	0x00000500},
	{0x03104A,	0x00000500},
	{0x058025,	0x00000000},
	{0x05802D,	0x00000000},
	{0x05802C,	0x00000000},
	{0x06E000,	0x00000001},
};
// [dwc_ddrphy_phyinit_D_loadIMEM, 1D] Start of dwc_ddrphy_phyinit_D_loadIMEM (Train2D
// Step (D) Load the 1D IMEM image
const	uint32_t	ddrphy_train1d_data_d[][2] =
{
	{0x058060,	0x00000002},
	{0x06E000,	0x00000000},
//	{0x06E000,	0x00000001},
};
// Step (E) Set the PHY input clocks to the desired frequency for pstate 0 
// [phyinit_F_loadDMEM, 1D] Start of dwc_ddrphy_phyinit_F_loadDMEM (pstate
// Step (F) Load the 1D DMEM image and write the 1D Message Block parameters for the training firmware 
const	uint32_t	ddrphy_train1d_data_f[][2] =
{
//	{0x06E000,	0x00000000},
	{0x06E000,	0x00000001},
};
// Step (G) Execute the Training Firmware 
//     ResetToMicro fields to 1 (all other fields should be zero). 
//     Then rewrite the CSR so that only the StallToMicro remains set (all other fields should be zero). 
const	uint32_t	ddrphy_train1d_data_g1[][2] =
{
	{0x06E000,	0x00000001},
	{0x06E099,	0x00000009},
	{0x06E099,	0x00000001},
	{0x06E099,	0x00000000},
};
// EXEC: dwc_ddrphy_phyinit_userCustom_G_waitFwDone ();
const	uint32_t	ddrphy_train1d_data_g2[][2] =
{
	{0x06E099,	0x00000001},
};
// [dwc_ddrphy_phyinit_G_execFW] End of dwc_ddrphy_phyinit_G_execFW ()
// Step (H) Read the Message Block results
const	uint32_t	ddrphy_train1d_data_h[][2] =
{
	{0x06E000,	0x00000000},
};
// if (Train1D)
// dwc_ddrphy_phyinit_userCustom_H_readMsgBlock (1);
// [dwc_ddrphy_phyinit_userCustom_H_readMsgBlock] End of dwc_ddrphy_phyinit_userCustom_H_readMsgBlock ()
// [dwc_ddrphy_phyinit_D_loadIMEM, 2D] Start of dwc_ddrphy_phyinit_D_loadIMEM (Train2D
// Step (D) Load the 2D IMEM image
const	uint32_t	ddrphy_train2d_data_d[][2] =
{
	{0x06E000,	0x00000000},
	{0x06E000,	0x00000000},
//	{0x06E000,	0x00000001},
};
// [phyinit_F_loadDMEM, 2D] Start of dwc_ddrphy_phyinit_F_loadDMEM (pstate
// Step (F) Load the 2D DMEM image and write the 2D Message Block parameters for the training firmware 
const	uint32_t	ddrphy_train2d_data_f[][2] =
{
//	{0x06E000,	0x00000000},
	{0x06E000,	0x00000001},
};
// Step (G) Execute the Training Firmware 
const	uint32_t	ddrphy_train2d_data_g1[][2] =
{
	{0x06E000,	0x00000001},
	{0x06E099,	0x00000009},
	{0x06E099,	0x00000001},
	{0x06E099,	0x00000000},
	{0x06E031,	0x00000000},
// Step (H) Read the Message Block results
	{0x06E000,	0x00000001},
	{0x06E099,	0x00000009},
	{0x06E099,	0x00000001},
	{0x06E099,	0x00000000},
};
// EXEC: dwc_ddrphy_phyinit_userCustom_G_waitFwDone ();
const	uint32_t	ddrphy_train2d_data_g2[][2] =
{
	{0x06E099,	0x00000001},
};
// [dwc_ddrphy_phyinit_G_execFW] End of dwc_ddrphy_phyinit_G_execFW ()
// Step (H) Read the Message Block results
const	uint32_t	ddrphy_train2d_data_h[][2] =
{
	{0x06E000,	0x00000000},
};
// if (Train2D)
// dwc_ddrphy_phyinit_userCustom_H_readMsgBlock (1);
// [dwc_ddrphy_phyinit_userCustom_H_readMsgBlock] End of dwc_ddrphy_phyinit_userCustom_H_readMsgBlock ()
// Step (I) Load PHY Init Engine Image 
const	uint32_t	ddrphy_train2d_data_i[][2] =
{
	{0x06E000,	0x00000000},
	{0x069000,	0x00000010},
	{0x069001,	0x00000400},
	{0x069002,	0x0000010e},
	{0x069003,	0x00000000},
	{0x069004,	0x00000000},
	{0x069005,	0x00000008},
	{0x069029,	0x0000000b},
	{0x06902A,	0x00000480},
	{0x06902B,	0x00000109},
	{0x06902C,	0x00000008},
	{0x06902D,	0x00000448},
	{0x06902E,	0x00000139},
	{0x06902F,	0x00000008},
	{0x069030,	0x00000478},
	{0x069031,	0x00000109},
	{0x069032,	0x00000002},
	{0x069033,	0x00000010},
	{0x069034,	0x00000139},
	{0x069035,	0x0000000b},
	{0x069036,	0x000007c0},
	{0x069037,	0x00000139},
	{0x069038,	0x00000044},
	{0x069039,	0x00000633},
	{0x06903A,	0x00000159},
	{0x06903B,	0x0000014f},
	{0x06903C,	0x00000630},
	{0x06903D,	0x00000159},
	{0x06903E,	0x00000047},
	{0x06903F,	0x00000633},
	{0x069040,	0x00000149},
	{0x069041,	0x0000004f},
	{0x069042,	0x00000633},
	{0x069043,	0x00000179},
	{0x069044,	0x00000008},
	{0x069045,	0x000000e0},
	{0x069046,	0x00000109},
	{0x069047,	0x00000000},
	{0x069048,	0x000007c8},
	{0x069049,	0x00000109},
	{0x06904A,	0x00000000},
	{0x06904B,	0x00000001},
	{0x06904C,	0x00000008},
	{0x06904D,	0x00000000},
	{0x06904E,	0x0000045a},
	{0x06904F,	0x00000009},
	{0x069050,	0x00000000},
	{0x069051,	0x00000448},
	{0x069052,	0x00000109},
	{0x069053,	0x00000040},
	{0x069054,	0x00000633},
	{0x069055,	0x00000179},
	{0x069056,	0x00000001},
	{0x069057,	0x00000618},
	{0x069058,	0x00000109},
	{0x069059,	0x000040c0},
	{0x06905A,	0x00000633},
	{0x06905B,	0x00000149},
	{0x06905C,	0x00000008},
	{0x06905D,	0x00000004},
	{0x06905E,	0x00000048},
	{0x06905F,	0x00004040},
	{0x069060,	0x00000633},
	{0x069061,	0x00000149},
	{0x069062,	0x00000000},
	{0x069063,	0x00000004},
	{0x069064,	0x00000048},
	{0x069065,	0x00000040},
	{0x069066,	0x00000633},
	{0x069067,	0x00000149},
	{0x069068,	0x00000010},
	{0x069069,	0x00000004},
	{0x06906A,	0x00000018},
	{0x06906B,	0x00000000},
	{0x06906C,	0x00000004},
	{0x06906D,	0x00000078},
	{0x06906E,	0x00000549},
	{0x06906F,	0x00000633},
	{0x069070,	0x00000159},
	{0x069071,	0x00000d49},
	{0x069072,	0x00000633},
	{0x069073,	0x00000159},
	{0x069074,	0x0000094a},
	{0x069075,	0x00000633},
	{0x069076,	0x00000159},
	{0x069077,	0x00000441},
	{0x069078,	0x00000633},
	{0x069079,	0x00000149},
	{0x06907A,	0x00000042},
	{0x06907B,	0x00000633},
	{0x06907C,	0x00000149},
	{0x06907D,	0x00000001},
	{0x06907E,	0x00000633},
	{0x06907F,	0x00000149},
	{0x069080,	0x00000000},
	{0x069081,	0x000000e0},
	{0x069082,	0x00000109},
	{0x069083,	0x0000000a},
	{0x069084,	0x00000010},
	{0x069085,	0x00000109},
	{0x069086,	0x00000009},
	{0x069087,	0x000003c0},
	{0x069088,	0x00000149},
	{0x069089,	0x00000009},
	{0x06908A,	0x000003c0},
	{0x06908B,	0x00000159},
	{0x06908C,	0x00000018},
	{0x06908D,	0x00000010},
	{0x06908E,	0x00000109},
	{0x06908F,	0x00000000},
	{0x069090,	0x000003c0},
	{0x069091,	0x00000109},
	{0x069092,	0x00000018},
	{0x069093,	0x00000004},
	{0x069094,	0x00000048},
	{0x069095,	0x00000018},
	{0x069096,	0x00000004},
	{0x069097,	0x00000058},
	{0x069098,	0x0000000b},
	{0x069099,	0x00000010},
	{0x06909A,	0x00000109},
	{0x06909B,	0x00000001},
	{0x06909C,	0x00000010},
	{0x06909D,	0x00000109},
	{0x06909E,	0x00000005},
	{0x06909F,	0x000007c0},
	{0x0690A0,	0x00000109},
	{0x0690A1,	0x00000000},
	{0x0690A2,	0x00008140},
	{0x0690A3,	0x0000010c},
	{0x0690A4,	0x00000010},
	{0x0690A5,	0x00008138},
	{0x0690A6,	0x00000104},
	{0x0690A7,	0x00000008},
	{0x0690A8,	0x00000448},
	{0x0690A9,	0x00000109},
	{0x0690AA,	0x0000000f},
	{0x0690AB,	0x000007c0},
	{0x0690AC,	0x00000109},
	{0x0690AD,	0x00000047},
	{0x0690AE,	0x00000630},
	{0x0690AF,	0x00000109},
	{0x0690B0,	0x00000008},
	{0x0690B1,	0x00000618},
	{0x0690B2,	0x00000109},
	{0x0690B3,	0x00000008},
	{0x0690B4,	0x000000e0},
	{0x0690B5,	0x00000109},
	{0x0690B6,	0x00000000},
	{0x0690B7,	0x000007c8},
	{0x0690B8,	0x00000109},
	{0x0690B9,	0x00000008},
	{0x0690BA,	0x00008140},
	{0x0690BB,	0x0000010c},
	{0x0690BC,	0x00000000},
	{0x0690BD,	0x00000478},
	{0x0690BE,	0x00000109},
	{0x0690BF,	0x00000000},
	{0x0690C0,	0x00000001},
	{0x0690C1,	0x00000008},
	{0x0690C2,	0x00000008},
	{0x0690C3,	0x00000004},
	{0x0690C4,	0x00000000},
	{0x069006,	0x00000008},
	{0x069007,	0x000007c8},
	{0x069008,	0x00000109},
	{0x069009,	0x00000000},
	{0x06900A,	0x00000400},
	{0x06900B,	0x00000106},
	{0x06E0E7,	0x00000400},
	{0x069017,	0x00000000},
	{0x069026,	0x0000002a},
	{0x05800B,	0x00000032},
	{0x05800C,	0x00000064},
	{0x05800D,	0x000003e8},
	{0x05800E,	0x0000002c},
	{0x06900C,	0x00000000},
	{0x06900D,	0x00000173},
	{0x06900E,	0x00000060},
	{0x06900F,	0x00006110},
	{0x069010,	0x00002152},
	{0x069011,	0x0000dfbd},
	{0x069012,	0x0000ffff},
	{0x069013,	0x00006152},
	{0x058089,	0x00000001},
	{0x058088,	0x00000019},
// [phyinit_I_loadPIEImage] Disabling Ucclk (PMU) and Hclk (training hardware)
	{0x06D080,	0x00000000},
	{0x06E000,	0x00000001},
};
// Step (J) Initialize the PHY to Mission Mode through DFI Initialization 

const uint32_t ddrphy_ini_data_c_size = ARRAY_SIZE(ddrphy_ini_data_c);
const uint32_t ddrphy_train1d_data_d_size = ARRAY_SIZE(ddrphy_train1d_data_d);
const uint32_t ddrphy_train1d_data_f_size = ARRAY_SIZE(ddrphy_train1d_data_f);
const uint32_t ddrphy_train1d_data_g1_size = ARRAY_SIZE(ddrphy_train1d_data_g1);
const uint32_t ddrphy_train1d_data_g2_size = ARRAY_SIZE(ddrphy_train1d_data_g2);
const uint32_t ddrphy_train1d_data_h_size = ARRAY_SIZE(ddrphy_train1d_data_h);
const uint32_t ddrphy_train2d_data_d_size = ARRAY_SIZE(ddrphy_train2d_data_d);
const uint32_t ddrphy_train2d_data_f_size = ARRAY_SIZE(ddrphy_train2d_data_f);
const uint32_t ddrphy_train2d_data_g1_size = ARRAY_SIZE(ddrphy_train2d_data_g1);
const uint32_t ddrphy_train2d_data_g2_size = ARRAY_SIZE(ddrphy_train2d_data_g2);
const uint32_t ddrphy_train2d_data_h_size = ARRAY_SIZE(ddrphy_train2d_data_h);
const uint32_t ddrphy_train2d_data_i_size = ARRAY_SIZE(ddrphy_train2d_data_i);
