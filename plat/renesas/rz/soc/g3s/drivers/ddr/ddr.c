...
#define	START_LIST_1D	0x0000
#define	START_LIST_2D	0x0800
#define	START_LIST_CTL	0x0C00
#define	SCR_SIZE		0x1000

static	uint32_t	csr_table[SCR_SIZE];

// get csr_table pointer
uint32_t ddr_get_csr_table(uint32_t	*csr_table_ptr[]);
// 11_initialization
void ddr_setup(void);
// 12_retention_entry
...
static void	save_rst_csr(void)
{
	...
	{
		csr_table[i] = 0xffffffff;
	}
	for(i = 0; i < ARRAY_SIZE(reg_list_1d); i++)
	{
		csr_table[START_LIST_1D + (i << 2)] = 	dwc_ddrphy_apb_rd(reg_list_1d[i]);
	}
	for(i = 0; i < ARRAY_SIZE(reg_list_2d); i++)
	{
		csr_table[START_LIST_2D + (i << 2)] = 	dwc_ddrphy_apb_rd(reg_list_2d[i]);
	}
	for(i = 0; i < ARRAY_SIZE(reg_list_ctl); i++)
	{
		csr_table[START_LIST_CTL + (i << 2)] = 	DDRTOP_mc_apb_rd(reg_list_ctl[i]);
	}
...
static void	restore_setcsr(void)
{
	...
	for(i = 0; i < ARRAY_SIZE(reg_list_1d); i++)
	{
		dwc_ddrphy_apb_wr(reg_list_1d[i], csr_table[START_LIST_1D + (i << 2)]);
	}
	for(i = 0; i < ARRAY_SIZE(reg_list_2d); i++)
	{
		dwc_ddrphy_apb_wr(reg_list_2d[i], csr_table[START_LIST_2D + (i << 2)]);
	}
	for(i = 0; i < ARRAY_SIZE(reg_list_ctl); i++)
	{
		DDRTOP_mc_apb_wr(reg_list_ctl[i], csr_table[START_LIST_CTL + (i << 2)]);
	}
	dwc_ddrphy_apb_wr(0x066080, 0);	// Write the UcclkHclkEnables CSR to disable the appropriate clocks after all reads done.
	dwc_ddrphy_apb_wr(0x06E000, 1);	// Write the MicroContMuxSel CSR to 0x1 to isolate the internal CSRs during mission mode.
}
