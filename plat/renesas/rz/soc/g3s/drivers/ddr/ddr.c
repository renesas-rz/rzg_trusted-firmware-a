...
#define	INT_STATUS_INIT_ADDR		220
...
static void phyinit(void)
{
	...
	// Should execute this step only when the training mode is SkipTrain.
...
static void phyinit_d2h_1d(void)
{
	...
	dwc_ddrphy_phyinit_userCustom_G_waitFwDone(0x1D);
...
static void phyinit_d2h_2d(void)
{
	...
	dwc_ddrphy_phyinit_userCustom_G_waitFwDone(0x2D);
...
{
	uint32_t i;
	dwc_ddrphy_apb_wr(0x0d0000, 0); // Write the MicroContMuxSel CSR to 0x0 to allow access to the internal CSRs.
	dwc_ddrphy_apb_wr(0x0c0080, 3); // Write the UcclkHclkEnables CSR to 0x3 to enable all the clocks so the reads can complete.
	...
	dwc_ddrphy_apb_wr(0x0c0080, 0); // Write the UcclkHclkEnables CSR to disable the appropriate clocks after all reads done.
	dwc_ddrphy_apb_wr(0x0d0000, 1); // Write the MicroContMuxSel CSR to 0x1 to isolate the internal CSRs during mission mode.
}
...
static uint32_t get_mail(void)
{
	...
	wd_timer2 = 0;
	do {
		rd_data = dwc_ddrphy_apb_rd(0x0d0004);
	}	while ((rd_data & 0x1) != 0);
	...
	do {
		rd_data = dwc_ddrphy_apb_rd(0x0d0004);
		INFO("INFO: Msg read. Waiting acknowledgement from uCtl ...\n");
