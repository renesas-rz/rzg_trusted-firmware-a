...
static void decode_streaming_message_lpddr4x(uint32_t codede_message_hex, uint16_t *args_list);
static void decode_streaming_message_lpddr4x_2d(uint32_t codede_message_hex, uint16_t *args_list);
...
static void decode_streaming_message(uint8_t sel_train)
{
	...
	num_args = (uint16_t)(codede_message_hex & 0xFFFF);
	...
	case 1:
		...
		break;
	case 2:
		decode_streaming_message_lpddr4x(codede_message_hex, args_list);
		break;
	case 3:
		decode_streaming_message_lpddr4x_2d(codede_message_hex, args_list);
		break;
	}
}
...
static void decode_streaming_message_lpddr4_2d(uint32_t codede_message_hex, uint16_t *args_list)
{
	...
	case 0x00020002:
		...
		break;
	case 0x00030002:
		INFO("PMU0: VrefIdx %d -> MR[6:0] 0x%02x\n", args_list[0], args_list[1]);
		break;
	case 0x00040001:
		INFO("PMU0: Converting MR 0x%04x to vrefIdx\n", args_list[0]);
		break;
	case 0x00050002:
	...
	case 0x001a0001:
		...
		break;
	case 0x001b0003:
		INFO("PMU0: tgToCsn: tg %d + 0x%04x -> csn %d\n", args_list[0], args_list[1], args_list[2]);
		break;
	case 0x001c0002:
		INFO("PMU: Error: LP4 rank %d cannot be mapped on tg %d\n", args_list[0], args_list[1]);
		break;
	case 0x001d0002:
		INFO("PMU3: Sending vref %d,  Mr = 0X%05x, to all devices\n", args_list[0], args_list[1]);
		break;
	case 0x001e0004:
		INFO("PMU4: -------- %dD Write Scanning TG %d (CS 0x%x) Lanes 0x%03x --------\n", args_list[0], args_list[1], args_list[2], args_list[3]);
		break;
	case 0x001f0002:
		INFO("PMU0: training lanes 0x%03x using lanes 0x%03x\n", args_list[0], args_list[1]);
		break;
	case 0x00200003:
		INFO("PMU4: ------- 2D-DFE Read Scanning TG %d (CS 0x%x) Lanes 0x%03x -------\n", args_list[0], args_list[1], args_list[2]);
		break;
	case 0x00210004:
		INFO("PMU4: ------- %dD Read Scanning TG %d (CS 0x%x) Lanes 0x%03x -------\n", args_list[0], args_list[1], args_list[2], args_list[3]);
		break;
	case 0x00220003:
	...
	case 0x00300002:
		...
		break;
	case 0x00310006:
		INFO("PMU3: 2D Delay   Ranges: OOPL[0x%04x,0x%04x], IP[0x%04x,0x%04x], OOPR[0x%04x,0x%04x]\n", args_list[0], args_list[1], args_list[2], args_list[3], args_list[4], args_list[5]);
		break;
	case 0x00320002:
	...
	case 0x00500002:
		...
		break;
	case 0x00510009:
		INFO("PMU3: new center @ (%3d, %3d). Moved (%2i, %2i) -- L %d, R %d, C %d, U %d, D %d\n", args_list[0], args_list[1], args_list[2], args_list[3], args_list[4], args_list[5], args_list[6], args_list[7], args_list[8]);
		break;
	case 0x00520003:
	...
	case 0x00620000:
		...
		break;
	case 0x00630002:
		INFO("PMU5: Weak 1 changed to pull-up %5d ohms, pull-down %5d ohms\n", args_list[0], args_list[1]);
		break;
	case 0x00640002:
		INFO("PMU5: Weak 0 changed to pull-up %5d ohms, pull-down %5d ohms\n", args_list[0], args_list[1]);
		break;
	case 0x00650003:
		INFO("PMU0: dlyMargin L %02d R %02d, min %02d\n", args_list[0], args_list[1], args_list[2]);
		break;
	case 0x00660003:
		INFO("PMU0: vrefMargin T %02d B %02d, min %02d\n", args_list[0], args_list[1], args_list[2]);
		break;
	case 0x00670002:
	...
	case 0x00740000:
		...
		break;
	case 0x00750005:
		INFO("PMU4: DB%d L%d   %3d   %3d  ->  %3d (DISCONNECTED)\n", args_list[0], args_list[1], args_list[2], args_list[3], args_list[4]);
		break;
	case 0x00760005:
		INFO("PMU4: DB%d L%d   %3d   %3d  ->  %3d\n", args_list[0], args_list[1], args_list[2], args_list[3], args_list[4]);
		break;
	case 0x00770005:
		INFO("PMU0: writing rxClkDelay for tg%d db%1d nib%1d to 0x%02x from eye[%02d] (DISCONNECTED)\n", args_list[0], args_list[1], args_list[2], args_list[3], args_list[4]);
		break;
	case 0x00780003:
		INFO("PMU: Error: Dbyte %d nibble %d's optimal rxClkDly of 0x%x is out of bounds\n", args_list[0], args_list[1], args_list[2]);
		break;
	case 0x00790005:
		INFO("PMU0: writing rxClkDelay for tg%d db%1d nib%1d to 0x%02x from eye[%02d]\n", args_list[0], args_list[1], args_list[2], args_list[3], args_list[4]);
		break;
	case 0x007a0005:
		INFO("PMU0: tx voltage for tg%2d nib%2d to %3d (%d) from eye[%02d]\n", args_list[0], args_list[1], args_list[2], args_list[3], args_list[4]);
		break;
	case 0x007b0001:
	...
	case 0x007c0004:
		...
		break;
	case 0x007d0007:
		INFO("PMU0: writing txDqDelay for tg%1d db%1d ln%1d to  0x%02x (%d coarse, %d fine) from eye[%02d] (DISCONNECTED)\n", args_list[0], args_list[1], args_list[2], args_list[3], args_list[4], args_list[5], args_list[6]);
		break;
	case 0x007e0003:
		INFO("PMU: Error: Dbyte %d lane %d's optimal txDqDly of 0x%x is out of bounds\n", args_list[0], args_list[1], args_list[2]);
		break;
	case 0x007f0007:
		INFO("PMU0: writing txDqDelay for tg%1d db%1d l%1d to  0x%02x (%d coarse, %d fine) from eye[%02d]\n", args_list[0], args_list[1], args_list[2], args_list[3], args_list[4], args_list[5], args_list[6]);
		break;
	case 0x00800002:
	...
	case 0x00830003:
		...
		break;
	case 0x00840001:
		INFO("PMU10: DB DFE feature not fully supported, F2BCEx value is 0x%02x\n", args_list[0]);
		break;
	case 0x00850001:
		INFO("PMU10: DB DFE feature fully supported, F2BCEx value is 0x%02x\n", args_list[0]);
		break;
	case 0x00860002:
		INFO("PMU8: Start d4_2d_lrdimm_rx_dfe for tap %d biasStepInc %d\n", args_list[0], args_list[1]);
		break;
	case 0x00870001:
		INFO("PMU7: Start d4_2d_lrdimm_rx_dfe tapCoff 0x%0x\n", args_list[0]);
		break;
	case 0x00880003:
		INFO("PMU6: d4_2d_lrdimm_rx_dfe db %d lane %d area %d\n", args_list[0], args_list[1], args_list[2]);
		break;
	case 0x00890004:
		INFO("PMU7: d4_2d_lrdimm_rx_dfe db %d lane %d max area %d best bias 0x%0x\n", args_list[0], args_list[1], args_list[2], args_list[3]);
		break;
	case 0x008a0001:
		INFO("PMU0: eye-lane %d is disable\n", args_list[0]);
		break;
	case 0x008b0003:
		INFO("PMU5: Setting 0x%x improved rank weight (%4d < %4d)\n", args_list[0], args_list[1], args_list[2]);
		break;
	case 0x008c0001:
