...
/**********************************************************************************************************************
* Function Name : r_sb_sb_verify_mac
**********************************************************************************************************************/
/*!****************************************************************************************************************//**
 * \ingroup     SBLIBSecureBootAPIsInternal
 * \brief       Validate the MAC of the manifest and image
 * \param       [in]   p_code_cert_st          Start address of Code Certificate Structure
 ...
 *        parse the TLV of the MAC tag and save the hash of the IMG PK of the code certificate
 *      - If build configuration #SB_CFG_IMAGE_ENC_DEC is enabled, supports image decryption
 *      - Implement the following as a glitch countermeasure :
 *        - Add a flow counter before and after calling R_CIP_DRV_PrcVerifyMac
 *
 * Pseudo-code for the function can be found below.
 * -# Initialize the variable ret with #SB_RET_ERR_INTERNAL_FAIL
 * -# Initialize the variable cip_ret with CIP_DRV_RET_FAIL
 * -# Set NULL to p_sign_pk
 * -# Set TLV search condition of Image Cipher Info to search_tlv_mac
 * -# <B>--- If #SB_CFG_SB_CERT_CHAIN_USE_IMG_PK is enabled ---</B>
 * -# Set TLV search conditions of MAC Info to search_tlv_mac
 ...
 *     -# To clear ret, set ret to #SB_RET_ERR_INTERNAL_FAIL
 *     -# <B>--- If #SB_CFG_SB_CERT_CHAIN_USE_IMG_PK is enabled ---</B>
 *     -# Set #SB_PRV_TLV_MAC_KEY_IMG_PK_IDX address of mac_tlvs to p_sign_pk
 *     -# <B>--- Endif #SB_CFG_SB_CERT_CHAIN_USE_IMG_PK is enabled ---</B>
 *     -# Check the obtained TLV with #sb_check_mac_tlv
 *     -# Set function result to ret
 *     -# If #SB_RET_SUCCESS == ret
 *       -# To clear ret, set ret to #SB_RET_ERR_INTERNAL_FAIL
 *       -# Set mac verification parameters with #r_sb_cmn_drv_set_mac_param
 *       -# Set function result to ret
 *       -# If #SB_RET_SUCCESS == ret
 *         -# To clear ret, set ret to #SB_RET_ERR_INTERNAL_FAIL
 ...
 *           -# Add #SB_PRV_CMN_FC_SECUREBOOT_CNT to flow counter with #r_sb_cmn_fc_add_counter
 *           -# <B>--- Endif #SB_CFG_IMAGE_ENC_DEC is enabled ---</B>
 *           -# Call SB-Driver API R_CIP_DRV_PrcVerifyMac
 *           -# Set SB-Driver API result to cip_ret
 *           -# Get the return code from the value of cpi_ret with #r_sb_cmn_drv_get_sb_ret_from_cip_ret
 *           -# Set function result to ret
 *           -# Add #SB_PRV_CMN_FC_SECUREBOOT_CNT to flow counter with #r_sb_cmn_fc_add_counter
 *           -# If #SB_RET_SUCCESS == ret
 *             -# To clear ret, set ret to #SB_RET_ERR_INTERNAL_FAIL to and set CIP_DRV_RET_FAIL to cip_ret
 *             -# Add #SB_PRV_CMN_FC_SECUREBOOT_CNT to flow counter with #r_sb_cmn_fc_add_counter
 *             -# Call SB-Driver API R_CIP_DRV_CheckImageVersion
 *             -# Set SB-Driver API result to cip_ret
 *             -# Get the return code from the value of cpi_ret with #r_sb_cmn_drv_get_sb_ret_from_cip_ret
...
#endif /* (SB_CFG_SB_MAC_VERIFICATION == 1U) */
/**********************************************************************************************************************
* End of function r_sb_sb_verify_mac()
**********************************************************************************************************************/
