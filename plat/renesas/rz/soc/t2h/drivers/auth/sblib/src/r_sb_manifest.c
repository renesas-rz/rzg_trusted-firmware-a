...
/**********************************************************************************************************************
* Function Name : r_sb_mani_set_mac_tlv_st
**********************************************************************************************************************/
/*!****************************************************************************************************************//**
 * \ingroup     SBLIBManifestAPIsInternal
 * \brief       Set value in MAC TLV structure
 * \param       [in]   p_mac_tlv            Start address of MAC TLV
 * \param       [out]  p_mac_tlv_st         Start address of MAC TLV Structure
 * \return
 *  - #SB_RET_SUCCESS:                          Successful completion
 *  - #SB_RET_ERR_INTERNAL_FAIL:                Internal processing failed
 * \par Global Variables
 ...
 *  - yes
 * \par Detailed description
 *      - Set value in MAC TLV structure
 *      - Set MAC TLV type, TLV Value size, TLV Value start address
 *
 * Pseudo-code for the function can be found below.
 * -# Initialize the variable ret with #SB_RET_ERR_INTERNAL_FAIL
 * -# If (NULL != p_mac_tlv) && (NULL != p_mac_tlv_st)
 *   -# Cast the type of the argument p_mac_tlv to uint32_t * type and set it to p_mac_tlv_st->type
 *   -# Set the length of the argument p_mac_tlv to byte length,
 *      cast it to uint32_t* type, and set it to p_mac_tlv_st->byte_len
 *   -# Cast the #SB_PRV_TLV_TL_SIZE address of the argument p_mac_tlv to uint32_t* type and set it to
 *      p_mac_tlv_st->val
 *   -# Set ret to #SB_RET_SUCCESS
 * -# Else
 *   -# Do nothing
