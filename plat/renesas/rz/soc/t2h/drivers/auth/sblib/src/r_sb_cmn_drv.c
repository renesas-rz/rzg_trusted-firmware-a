...
/**********************************************************************************************************************
* Function Name : r_sb_cmn_drv_set_mac_param
**********************************************************************************************************************/
/*!****************************************************************************************************************//**
 * \ingroup     SBLIBCommonAPIsInternal
 * \brief       Setting MAC Verification parameters
 * \param       [in]   p_code_cert_st         Start address of Code Certificate Structure
 * \param       [in]   p_mac_tlv              Start address of MAC TLV
 * \param       [in]   p_sign_pk_tlv          Start address of Signature public key TLV
 * \param       [out]  p_mac_param            Starting address of the structure for setting MAC verify parameters
 * \return
 *  - #SB_RET_SUCCESS:                             Successful completion
 *  - #SB_RET_ERR_INTERNAL_FAIL:                   Internal processing failed
 * \par Global Variables
 ...
 * Pseudo-code for the function can be found below.
 * -# Initialize the variable ret with #SB_RET_ERR_INTERNAL_FAIL
 * -# Initialize the variable ret_ck_pk with #SB_RET_ERR_INTERNAL_FAIL
 * -# If (NULL != p_code_cert_st) && (NULL != p_code_cert_st->p_header) && (NULL != p_mac_tlv) && (NULL != p_mac_param)
 *   -# Get MAC algorithm with #cmn_drv_get_mac_algo
 *   -# Set p_mac_param->mac_algo to function result
 *   -# Cast the p_mac_tlv->p_val to uint32_t* type and set it to p_mac_param->p_mac
 *   -# Set p_mac_param->mac_len to p_mac_tlv->byte_len
 *   -# Cast the p_code_cert_st->p_header to uint32_t* type and set it to p_mac_param->p_mac
 *   -# Calculate the size of the code certificate and set it to p_cc_code_cert_param->code_cert_len
 *   -# Set img_addr to p_code_cert_st->p_header->dest_addr
 *   -# Set p_mac_param->p_img to img_addr
 *   -# Set p_mac_param->img_len to p_code_cert_st->p_header->img_len
 *   -# <B>--- If #SB_CFG_SB_CERT_CHAIN_USE_IMG_PK is enabled ---</B>
 *   -# If (p_code_cert_st->p_header->flags & #SB_PRV_CODE_CERT_HEADER_FLAGS_SAVE_IMG_PK) == 1UL
 *     -# Set p_cc_code_cert_param->is_save_img_pk to 1
 *   -# Else
 *     -# Set p_cc_code_cert_param->is_save_img_pk to 0
 *   -# <B>--- Else #SB_CFG_SB_CERT_CHAIN_USE_IMG_PK is enabled ---</B>
 *   -# Set p_cc_code_cert_param->is_save_img_pk to 0
 *   -# <B>--- Endif #SB_CFG_SB_CERT_CHAIN_USE_IMG_PK is enabled ---</B>
 *   -# If 1UL == p_mac_param->is_save_img_pk
 *     -# If p_sign_pk_tlv != NULL
 *       -# Set p_mac_param->p_sign_pk to p_sign_pk_tlv->p_val
 *       -# Set p_mac_param->sign_pk_len to p_sign_pk_tlv->byte_len
 *       -# Set ret_ck_pk to #SB_RET_SUCCESS
 *     -# Else
 *       -# Do nothing
 *   -# Else
 *     -# Set p_mac_param->p_sign_pk to NULL
 *     -# Set p_mac_param->sign_pk_len to 0
 *     -# Set ret_ck_pk to #SB_RET_SUCCESS
...
/**********************************************************************************************************************
* Function Name : cmn_drv_get_mac_algo
**********************************************************************************************************************/
/*!****************************************************************************************************************//**
 * \ingroup     SBLIBCommonAPIsInternal
 * \brief       Get MAC Algorithm
 * \param       [in]   type                   MAC algorithm type
 * \return
 *  - CIP_DRV_MAC_ALGO_HMAC_SHA2_224:             MAC algorithm SHA2-224
 *  - CIP_DRV_MAC_ALGO_HMAC_SHA2_256:             MAC algorithm SHA2-256
 *  - CIP_DRV_MAC_ALGO_HMAC_SHA2_384:             MAC algorithm SHA2-384
 *  - CIP_DRV_MAC_ALGO_HMAC_SHA2_512:             MAC algorithm SHA2-512
 *  - CIP_DRV_MAC_ALGO_HMAC_SHA3_224:             MAC algorithm SHA3-224
 *  - CIP_DRV_MAC_ALGO_HMAC_SHA3_256:             MAC algorithm SHA3-256
 *  - CIP_DRV_MAC_ALGO_HMAC_SHA3_384:             MAC algorithm SHA3-384
 *  - CIP_DRV_MAC_ALGO_HMAC_SHA3_512:             MAC algorithm SHA3-512
 *  - CIP_DRV_MAC_ALGO_CMAC_AES_128:              MAC algorithm AES-128
 *  - CIP_DRV_MAC_ALGO_CMAC_AES_192:              MAC algorithm AES-192
 *  - CIP_DRV_MAC_ALGO_CMAC_AES_256:              MAC algorithm AES-256
 *  - SB_PRV_RET_UNSUPPORTED_ALGO:                Unsupported algorithm specified
 * \par Global Variables
 *  - None
 * \par Call SB-Driver API
 ...
 *      - Returns the MAC algorithm according to the argument
 *      - Convert SB-Lib MAC type to Crypto IP-Driver MAC algorithm value
 *      - This function gets the MAC algorithm value of Crypto IP-Driver from the MAC algorithm type in the TLV field
 *
 * Pseudo-code for the function can be found below.
 * -# Initialize the variable ret with #SB_PRV_RET_UNSUPPORTED_ALGO
 * -# switch (type & #SB_PRV_TLV_TYPE_CRYPTO_ALGO_MASK)
 *   -# type == #SB_PRV_TLV_TYPE_CRYPTO_ALGO_HMAC_SHA2_224, Set CIP_DRV_MAC_ALGO_HMAC_SHA2_224 to mac_algo
 *   -# type == #SB_PRV_TLV_TYPE_CRYPTO_ALGO_HMAC_SHA2_256, Set CIP_DRV_MAC_ALGO_HMAC_SHA2_256 to mac_algo
 *   -# type == #SB_PRV_TLV_TYPE_CRYPTO_ALGO_HMAC_SHA2_384, Set CIP_DRV_MAC_ALGO_HMAC_SHA2_384 to mac_algo
 *   -# type == #SB_PRV_TLV_TYPE_CRYPTO_ALGO_HMAC_SHA2_512, Set CIP_DRV_MAC_ALGO_HMAC_SHA2_512 to mac_algo
 *   -# type == #SB_PRV_TLV_TYPE_CRYPTO_ALGO_HMAC_SHA3_224, Set CIP_DRV_MAC_ALGO_HMAC_SHA3_224 to mac_algo
 *   -# type == #SB_PRV_TLV_TYPE_CRYPTO_ALGO_HMAC_SHA3_256, Set CIP_DRV_MAC_ALGO_HMAC_SHA3_256 to mac_algo
 *   -# type == #SB_PRV_TLV_TYPE_CRYPTO_ALGO_HMAC_SHA3_384, Set CIP_DRV_MAC_ALGO_HMAC_SHA3_384 to mac_algo
 *   -# type == #SB_PRV_TLV_TYPE_CRYPTO_ALGO_HMAC_SHA3_512, Set CIP_DRV_MAC_ALGO_HMAC_SHA3_512 to mac_algo
 *   -# type == #SB_PRV_TLV_TYPE_CRYPTO_ALGO_CMAC_AES128,   Set CIP_DRV_MAC_ALGO_CMAC_AES_128 to mac_algo
 *   -# type == #SB_PRV_TLV_TYPE_CRYPTO_ALGO_CMAC_AES192,   Set CIP_DRV_MAC_ALGO_CMAC_AES_192 to mac_algo
 *   -# type == #SB_PRV_TLV_TYPE_CRYPTO_ALGO_CMAC_AES256,   Set CIP_DRV_MAC_ALGO_CMAC_AES_256 to mac_algo
 *   -# In the default case,                                do nothing
 * -# Return sign_scheme
 *
 * \callgraph
