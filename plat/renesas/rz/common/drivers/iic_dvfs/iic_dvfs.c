...
#define RZ_DVFS_API(__name, ...)					\
	int32_t rz_iic_dvfs_ ##__name(__VA_ARGS__)
...
RZ_DVFS_API(send, uint8_t slave, uint8_t reg_addr, uint8_t reg_data)
{
...
RZ_DVFS_API(receive, uint8_t slave, uint8_t reg, uint8_t *data)
{
	enum dvfs_state_t state = DVFS_START;
	int32_t result = DVFS_PROCESS;
	uint32_t err = 0U;
