...
int32_t sd_mount(int32_t sd_port, uint32_t mode, uint32_t voltage)
{
	...
	/* ==== initialise card - first attempt ==== */
	udelay(100);
	if (_sd_card_init(p_hndl) != SD_OK) {

		/* ---- Stop clock before power off ---- */
		/* Prevents back-powering card through data lines */
		if (_sd_set_clock(p_hndl, 0, SD_CLOCK_DISABLE) != SD_OK) {
			return _sd_mount_error(p_hndl);
		}
		...
		/* ---- Step 3: Wait for card to fully discharge ---- */
		mdelay(250);
		...
		/* ---- Step 5: Wait for VCC to fully rise ---- */
		mdelay(10);

		/* ---- Step 6: Restart clock at 400KHz ---- */
		if (_sd_set_clock(p_hndl, SD_CLK_400KHZ, SD_CLOCK_ENABLE) != SD_OK) {
			return _sd_mount_error(p_hndl);
		}

		/* ---- Step 7: Wait for 74 clock cycles per SD spec ---- */
		mdelay(10);
		sddev_int_wait(sd_port, 2);

		/* ---- Step 8: Re-initialise registers ---- */
		sddev_loc_cpu(sd_port);
		...
		SDMMC.SD_INFO1.LONGLONG = (uint64_t)info1_back;
		SDMMC.SD_INFO2.LONGLONG = (uint64_t)0;
		SDMMC.CC_EXT_MODE.LONGLONG = (uint64_t)(SDMMC.CC_EXT_MODE.LONGLONG
												& ~CC_EXT_MODE_DMASDRW);

		sddev_unl_cpu(sd_port);

		/* ---- Step 9: Reset handle and retry card init ---- */
		p_hndl->error = SD_OK;
		udelay(100);
...
int32_t _sd_card_init(st_sdhndl_t *p_hndl)
{
	...
	if (SD_MEDIA_UNKNOWN == p_hndl->media_type) {
		...
		if (SD_OK != ret) {
			return SD_ERR;      /* sd_mount() will handle power cycle */
		}
		...
		/* Compute CMD8 argument every time */
		/* p_hndl->if_cond[] is overwritten with the CMD8 R7 response */
		cmd8_arg_low = _sd_calc_cmd8_arg(p_hndl->voltage);

		ret = _sd_card_send_cmd_arg(p_hndl, CMD8, SD_RSP_R7,
									 ARG_CMD8_ARG_HIGH, cmd8_arg_low);
			...
			if (SD_OK == ret) {
				...
				p_hndl->sd_spec = SD_SPEC_20;   /* cmd8 has response        */
												/* phys spec ver2.00        */
			} else {
				/* ==== clear illegal command error for CMD8 ==== */
				/* CMD8 not responding is normal for v1.x cards  */
				for (i = 0; i < 3; i++) {
					ret = _sd_send_cmd(p_hndl, CMD0);
					if (SD_OK == ret) {
						break;
				...
				p_hndl->error = SD_OK;
				p_hndl->sd_spec = SD_SPEC_10;   /* cmd8 has no response     */
												/* phys spec ver1.01/1.10   */
			}

		} else {
			p_hndl->sd_spec = SD_SPEC_10;       /* SD_MODE_VER1X            */
												/* phys spec ver1.01/1.10   */
		}
	} /* closes: if (SD_MEDIA_UNKNOWN == p_hndl->media_type) */
