...
#define SDMMC    (*(volatile struct st_sdmmc0 *)((uintptr_t)p_hndl->reg_base)) /* For SDHI register address */
...
/* CMD8 (SEND_IF_COND) argument, phys spec ver2.00: */
/* [31:12] reserved, [11:8] VHS (voltage supplied), [7:0] check pattern */
#define ARG_CMD8_CHECK_PATTERN      (0x00aau)   /* fixed check pattern, always 0xAA */
#define ARG_CMD8_VHS_HIGH_VOLTAGE   (0x0100u)   /* VHS bit: 2.7-3.6V */
#define ARG_CMD8_VHS_LOW_VOLTAGE    (0x0200u)   /* VHS bit: 1.65-1.95V (UHS-II style) */
#define ARG_CMD8_ARG_HIGH           (0x0000u)   /* upper 16 bits [31:16] are always reserved/0 */
...
#define SD_QUERY_PARTITION_SIZE     (512)
...
/* Function Name: _sd_calc_cmd8_arg */
/**************************************************************************//**
 * @fn            uint16_t _sd_calc_cmd8_arg(uint32_t voltage)
 * @brief         calculate CMD8 (SEND_IF_COND) argument low 16 bits. <br>
 *                combines the fixed 0xAA check pattern with the VHS
 *                (voltage supplied) bits derived from the working voltage.
 * @warning       .
 * @param [in]    uint32_t voltage : working voltage (p_hndl->voltage)
 * @retval        CMD8 argument, low 16 bits [15:0]
 *****************************************************************************/
uint16_t _sd_calc_cmd8_arg(uint32_t voltage);
