...
/******************************************************************************
 * Function Name: sdcalc_cmd8_arg
 * Description  : calculate CMD8 (SEND_IF_COND) argument low 16 bits.
 *              : combines the fixed 0xAA check pattern with the VHS
 *              : (voltage supplied) bits derived from the working voltage.
 * Arguments    : uint32_t voltage : working voltage (p_hndl->voltage)
 * Return Value : CMD8 argument, low 16 bits [15:0]
 *****************************************************************************/
uint16_t _sd_calc_cmd8_arg(uint32_t voltage)
{
	uint16_t arg_low;

	arg_low = ARG_CMD8_CHECK_PATTERN;

	if (voltage & 0x00FF8000) {
		arg_low |= ARG_CMD8_VHS_HIGH_VOLTAGE;   /* 2.7-3.6V */
	}
	if (voltage & 0x00000F00) {
		arg_low |= ARG_CMD8_VHS_LOW_VOLTAGE;    /* 1.65-1.95V */
	}

	return arg_low;
}
/******************************************************************************
 End of function sdcalc_cmd8_arg
 *****************************************************************************/
