...
int32_t _sd_send_cmd(st_sdhndl_t *p_hndl, uint16_t cmd)
{
	...
	_sd_set_int_mask(p_hndl, SD_INFO1_MASK_RESP, 0); //SD_INFO2_MASK_ERR);
	...
	if (sddev_int_wait(p_hndl->sd_port, time) != SD_OK) {
		...
		/* Cast to an appropriate type */
		_sd_clear_int_mask(p_hndl, SD_INFO1_MASK_RESP, SD_INFO2_MASK_ERR);
		_sd_get_info2(p_hndl);   /* also clear any latched HW error bits on timeout */
		return p_hndl->error;
	}
...
int32_t _sd_card_send_ocr(st_sdhndl_t *p_hndl, int32_t type)
{
	...
	for (i = 0; i < 200; i++) {
		...
		case SD_MEDIA_SD:
		case SD_MEDIA_COMBO:
			/* Add extra settling delay for cards that need more time */
			/* after CMD0 reset before accepting APP commands */
			if (i == 0) {
				sddev_int_wait(p_hndl->sd_port, 50); /* 50ms extra settling time */
			}
		...
		if (SD_ERR_RES_TOE == p_hndl->error) {
			++j;
			sddev_int_wait(p_hndl->sd_port, 20);
			if (3 == j) {
				break;
			}
		} else {
