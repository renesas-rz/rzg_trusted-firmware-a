...
static int32_t _sd_single_read(SDHNDL *p_hndl, uint8_t *buff, uint32_t psn, int32_t mode);
...
int32_t esd_read_sect(uint8_t *buff, uint32_t psn, int32_t cnt)
{
	...
	for (i = cnt; i > 0; i -= TRANS_SECTORS, psn += TRANS_SECTORS, buff += (TRANS_SECTORS * 512)) {
