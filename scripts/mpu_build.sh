...
{
	U_BOOT_FILE=""
	if [[ "$BOARD" = "smarc_1" ]] && [[ "$PIPELINE_TYPE" = "release" || "$PIPELINE_TYPE" = "merge" ]]; then
		U_BOOT_FILE="../../u-boot/g2lc-smarc-1-u-boot.bin"
	elif [[ "$BOARD" = "smarc_1" ]]; then
		U_BOOT_FILE="../u-boot/g2lc-smarc-1-u-boot.bin"
	else
		echo "Invalid G2LC board: Board doesn't exist"
		exit 1
	...
	# Hard coding the build to use g2l because that is the make command for g2lc
	run_command "make PLAT=g2l BOARD=$BOARD ""$CONFIGS"" BL33="$U_BOOT_FILE" bl2 fip bptool pkg" "$ERROR_MSG"
}
...
{
	U_BOOT_FILE=""
	if [[ "$BOARD" = "g2ul_smarc" ]] && [[ "$PIPELINE_TYPE" = "release" || "$PIPELINE_TYPE" = "merge" ]]; then
		U_BOOT_FILE="../../u-boot/g2ul-smarc-u-boot.bin"
	elif [[ "$BOARD" = "g2ul_smarc" ]]; then
		U_BOOT_FILE="../u-boot/g2ul-smarc-u-boot.bin"
	else
		echo "Invalid G2UL board: Board doesn't exist"
		exit 1
...
{
	U_BOOT_FILE=""
	if [[ "$BOARD" = "smarc_pmic_2" ]] && [[ "$PIPELINE_TYPE" = "release" || "$PIPELINE_TYPE" = "merge" ]]; then
		U_BOOT_FILE="../../u-boot/v2l-smarc-pmic-2-u-boot.bin"
	elif [[ "$BOARD" = "smarc_pmic_2" ]]; then
		U_BOOT_FILE="../u-boot/v2l-smarc-pmic-2-u-boot.bin"
	else
		echo "Invalid V2L board: Board doesn't exist"
		exit 1
